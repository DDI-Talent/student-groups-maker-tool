scenarios_operations <- list(
  run_one_session = function( people, group_size, history_until_now, session_name, how_many_alternatives = 50 ){
    fittest_option_of_next_session <- scenarios_operations$find_fittest_pairs(people = people,
                                          group_size = group_size,
                                          session_name = session_name,
                                          how_many_alternatives = how_many_alternatives,
                                          history_until_now=history_until_now)
    fittest_option_of_next_session
  },
  run_scenario = function( people, group_size, how_many_sessions, how_many_alternatives  ){
    ...
    for (which_session in 1:how_many_sessions) {
      cands_this_session <- scenarios_operations$find_fittest_pairs(people = people,
                                            group_size = group_size,
